...
from inversion.psp.mtcnn.mtcnn import MTCNN
from inversion.psp.encoders.model_irse import IR_101
...
def run(args):
    for step in sorted(os.listdir(args.output_path)):
        # if not step.isdigit():
        # 	continue
        outputs_path = os.path.join(args.output_path, step, 'sv_can')
        gt_path = os.path.join(args.gt_path, step, 'gt')
        # if os.path.isdir(step_outputs_path):
        # 	print('#' * 80)
        # 	print(f'Running on step: {step}')
        # 	print('#' * 80)
        run_on_step_output(output_path=outputs_path, gt_path=gt_path, args=args)
...
def run_on_step_output(output_path, gt_path, args):
    ...
    for f in os.listdir(step_outputs_path):
        if f.endswith('seg.png'):
            continue
        image_path = os.path.join(step_outputs_path, f)
        gt_path_2 = os.path.join(gt_path, "gt_"+f.split('_')[-1])
        if f.endswith(".jpg") or f.endswith('.png') or f.endswith('.jpeg'):
            file_paths.append([image_path, gt_path_2])
    ...
    with open(os.path.join(out_path, f'stat_id_step_sv_can.txt'), 'w') as f:
        f.write(result_str)
    with open(os.path.join(out_path, f'scores_id_step_sv_can.json'), 'w') as f:
