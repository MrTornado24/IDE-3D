...
def project(
    ...
    if start_w.shape[1] != G.mapping.num_ws:
        start_w = np.repeat(start_w, G.mapping.num_ws, axis=1)
    w_opt = torch.tensor(start_w, dtype=torch.float32, device=device,
                         requires_grad=True)  # pylint: disable=not-callable

    # w_geo = torch.tensor(start_w[:, :8], dtype=torch.float32, device=device,
    #                      requires_grad=True)
    # w_app = torch.tensor(start_w[:, 8:], dtype=torch.float32, device=device,
    #                      requires_grad=True)  # pylint: disable=not-callable
